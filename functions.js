...
properties = {
  ...
  singleResultsFile: {
    ...
    description:
      "Set to false if you want to store the measurement results for each probe / inspection toolpath in a separate file",
    group: "probing",
    type: "boolean",
    value: true,
    scope: "post",
...
var diameterOffsetFormat = createFormat({
  prefix: "D",
  width: 2,
  zeropad: true,
  decimals: 1,
...
function setSmoothing(mode) {
  ...
  if (
    typeof lengthCompensationActive != "undefined" &&
    smoothingSettings.cancelCompensation
  ) {
    validate(
...
function onSection() {
  ...
  if (isProbeOperation()) {
    validate(
      settings.probing.probeAngleMethod != "G68",
      "You cannot probe while G68 Rotation is in effect."
    );
    validate(
      settings.probing.probeAngleMethod != "G54.4",
    ...
    if (
      ...
      if (!_section.isOptimizedForMachine()) {
        machineConfiguration.setToolLength(
          compensateToolLength ? _section.getTool().overallLength : 0
...
function positionABC(abc, force) {
  ...
  var a = machineConfiguration.isMultiAxisConfiguration()
    ? aOutput.format(abc.x)
    : toolVectorOutputI.format(abc.x);
  var b = machineConfiguration.isMultiAxisConfiguration()
    ? bOutput.format(abc.y)
    : toolVectorOutputJ.format(abc.y);
  var c = machineConfiguration.isMultiAxisConfiguration()
    ? cOutput.format(abc.z)
    : toolVectorOutputK.format(abc.z);
  if (a || b || c) {
    if (!retracted) {
...
function unwindABC(abc) {
  ...
  for (var i in axes) {
    if (
      axes[i].isEnabled() &&
      (settings.unwind.useAngle != "prefix" ||
        settings.unwind.anglePrefix[axes[i].getCoordinate] != "")
    ) {
      ...
      var nearestABC = machineConfiguration.getABCByPreference(
        orientation,
        tempABC,
        ABC,
        PREFER_PREFERENCE,
      ...
      if (settings.unwind.method == 2) {
        distanceOrigin = abcFormat.getResultingValue(
          Math.abs(Vector.diff(nearestABC, abc).getCoordinate(j))
        );
      } else {
        // closest angle
        distanceOrigin =
          abcFormat.getResultingValue(
            Math.abs(getCurrentDirection().getCoordinate(j))
          ) % 360; // calculate distance for unwinding axis
        distanceOrigin =
          distanceOrigin > 180 ? 360 - distanceOrigin : distanceOrigin; // take shortest route to 0
        distanceOrigin += abcFormat.getResultingValue(
          Math.abs(abc.getCoordinate(j))
        ); // add distance from 0 to new position
      }
      ...
      if (
        distanceABC > distanceOrigin &&
        (settings.unwind.method == 2 || revolutions > 1)
      ) {
        // G28 method will move rotary, so make sure move is greater than 360 degrees
        if (!retracted) {
          if (typeof moveToSafeRetractPosition == "function") {
            moveToSafeRetractPosition();
          } else {
        ...
        writeBlock(
          ...
          settings.unwind.workOffsetCode
            ? settings.unwind.workOffsetCode + currentWorkOffset
            : "",
          settings.unwind.useAngle == "true"
            ? outputs[j].format(angle)
            : settings.unwind.useAngle == "prefix"
            ? settings.unwind.anglePrefix[j] + abcFormat.format(angle)
            : ""
        );
...
function subprogramStart(initialPosition, abc, incremental) {
  if (getProperty("useFilesForSubprograms")) {
    var path = FileSystem.getCombinedPath(
      ...
      subprogramState.currentSubprogram + "." + extension
    );
    redirectToFile(path);
  ...
  writeln(
    settings.subprograms.startBlock.embedded +
      settings.subprograms.format.format(subprogramState.currentSubprogram) +
      conditional(
        comment,
        SP +
          formatComment(
            comment.substring(
              0,
              settings.comments.maximumLineLength - 2 - 6 - 1
            )
          )
      )
  );
  ...
  if (subprogramState.lastSubprogram == undefined) {
    ...
    if (settings.subprograms.initialSubprogramNumber == undefined) {
      ...
      subprogramState.lastSubprogram =
        settings.subprograms.initialSubprogramNumber;
    }
  }
...
function writeRetract() {
  ...
  if (retract && retract.words.length > 0) {
    if (
      ...
      settings.retract.cancelRotationOnRetracting
    ) {
...
function writeInitialPositioning(position, isRequired, codes1, codes2) {
  ...
  var gOffset = getSetting("outputToolLengthCompensation", true)
    ? gFormat.format(getOffsetCode())
    : "";
  ...
  writeStartBlocks(isRequired, function () {
    ...
    if (
      ...
      if (angles.isNonZero()) {
        ...
        writeBlock(
          modalCodes,
          gMotionModal.format(motionCode),
...
function writeProgramNumber() {
  if (programName) {
    ...
    if (
      !(programId >= 1 && (programId <= getProperty("o8") ? 99999999 : 9999))
    ) {
...
function writeDrillCycle(cycle, x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    var F = getProperty("useG95")
      ? cycle.feedrate / spindleSpeed
      : cycle.feedrate;
...
function inspectionCreateResultsFileHeader() {
  ...
  if (!isDPRNTopen) {
    ...
    if (hasGlobalParameter("model-version")) {
      writeln(
        "DPRNT[MODELVERSION*" +
          getGlobalParameter("model-version").toUpperCase() +
          "]"
      );
    }
  }
...
function writeProbingToolpathInformation(cycleDepth) {
  ...
  if (isInspectionOperation()) {
    writeln(
      "DPRNT[TOOLPATH*" +
        getParameter("operation-comment").toUpperCase().replace(/[()]/g, "") +
        "]"
    );
  } else {
...
function writeProbeCycle(cycle, x, y, z, P, F) {
  if (isProbeOperation()) {
    if (
      ...
      if (
        !settings.probing.allowIndexingWCSProbing &&
        currentSection.strategy == "probe"
      ) {
        error(
          localize(
  ...
  switch (cycleType) {
    ...
    case "probing-xy-inner-corner":
      var cornerX =
        x +
        approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2);
      var cornerY =
        y +
        approach(cycle.approach2) * (cycle.probeClearance + tool.diameter / 2);
      var cornerI = 0;
      var cornerJ = 0;
...
function setProbeAngleMethod() {
  settings.probing.probeAngleMethod =
    machineConfiguration.getNumberOfAxes() < 5 || is3D()
      ? getProperty("useG54x4")
        ? "G54.4"
        : "G68"
      : "UNSUPPORTED";
  var axes = [
